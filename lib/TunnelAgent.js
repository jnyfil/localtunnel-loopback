...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        this.debug = Debug(`lt:TunnelAgent[${options.id}]`);
    ...
    listen(port = null) {
        ...
        return new Promise((resolve) => {
            server.listen(port, () => {
                ...
                console.info('tunnel listening on port: %d', port);
    ...
    _onConnection(socket) {
        ...
        if (!remoteAddr) {
            this.debug('Remote address not available');
            socket.destroy();
            return false;
        }

        if (!ip.isEqual(this.ownerIp, remoteAddr)) {
            this.debug(`Unauthorized remote IP address : ${remoteAddr}`);
            socket.destroy();
            return false;
        }
